        ...
        if (self.showField):
            for x in range(-200, 200, 10):
                for y in range(-200, 200, 10):
                    forceVector = self.calculateElectricField(self, vector(x, y, 0))
                    arrow(pos=vector(x, y, 0), axis=forceVector, color=color.green)
...
mouse = StupidMouse()
charge = Charges(vector(10, 0, 0), 1, color.red, False)
forceCreatorsList = []
