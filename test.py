...
class Puck:
    ...
    def update(self, forcer, obstacleList, chargeList, goal):
        ...
        for o in goal.shape:
            self.touchingGoal(o)
        # print(self.velocity)
        frictionalForce = - (norm(self.netForce)*self.friction*9.81)
        print(self.netForce)
        print(frictionalForce)
        if ((mag(self.netForce) + mag(frictionalForce) ) < 0):
            frictionalForce = -self.netForce
        self.velocity = self.velocity + ((self.netForce + frictionalForce)/ self.mass) * (1 / runRate)
...
slider(bind=changeFriction, min=0, max=10, step=1, pos=scene.caption_anchor)
